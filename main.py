...
async def bind_from_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2) الصيغة الجديدة (Forwarded Messages 2.0)
    elif getattr(msg, "forward_origin", None):
        try:
            # MessageOriginChannel / MessageOriginChat يحملان chat
            fchat = msg.forward_origin.chat  # قد يرمي استثناء إن كان origin من user/public service
    ...
    if not fchat:
        await msg.reply_text(
            "⚠️ هذه ليست *إعادة توجيه* تحتوي على مصدر القناة.\n"
            "جرّب التحويل مباشرة من القناة (بدون نسخ/Copy)، وتأكد تعطيل خيار *Hide sender name*.",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    # تأكد أن البوت يقدر يصل للقناة (مضاف/أدمن)
    try:
        await context.bot.get_chat(fchat.id)
    except Exception:
        await msg.reply_text(
            "⚠️ لا أستطيع الوصول إلى القناة.\n"
            "أضِفني كـ *مشرف* في القناة أولًا ثم أعد التوجيه.",
            parse_mode=ParseMode.MARKDOWN
        )
        return
