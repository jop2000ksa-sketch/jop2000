# Write a corrected, Render-ready webhook version of main.py as main_render.py
...
PORT = int(os.getenv("PORT", "10000"))  # Render يمرر PORT
...
async def bind_from_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not fchat or fchat.type not in ("channel", "supergroup", "group"):
        return  # نتجاهل أي شيء ليس معاد توجيهه من قناة/مجموعة

    # تحقق أن المرسل أدمن في هذه القناة/المجموعة
    try:
        member = await context.bot.get_chat_member(fchat.id, user.id)
        if member.status not in ("administrator", "creator"):
            await msg.reply_text("⚠️ يجب أن تكون مشرفًا في القناة المعاد توجيهها.")
            return
    except Exception:
        await msg.reply_text("⚠️ تعذّر التحقق من صلاحيتك هناك.")
        return

    # اربط القناة/المجموعة لهذا المشرف
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # لا نتدخل في جلسة الرد
    if context.bot_data.get("current_reply"):
        return

    # تشغيل النشر بكتابة JOP كنص (بدون /) — قبل أي حفظ محتوى
    msg = (update.message.text or "").strip()
    if re.fullmatch(r"\s*jop\s*", msg, flags=re.IGNORECASE):
        await start_publish(update, context)
        return

    # فقط الأدمن المرتبط بوجهة نشر
    if not await is_user_admin(update, context):
        return

    # لو الأدمن داخل جلسة استفسار كمستخدم → لا نلمس
    ...
    # لا تظهر رسالة "تم حفظ النص" إلا بعد هذا الإدخال (وهذا هو الإدخال)
...
application.add_handler(MessageHandler(filters.FORWARDED & filters.ChatType.PRIVATE, bind_from_forward), group=0)

# 🟢 رسائل المستخدم (الاستفسار) — داخل الخاص فقط وبدون أوامر
...
async def on_startup():
    # شغّل تطبيق PTB (دون polling)
    await application.initialize()
    await application.start()

    if not APP_URL:
        # في أول تشغيل قد يتأخر تعيين RENDER_EXTERNAL_URL، يمكنك إعادة النشر لتسجيل الويبهوك لاحقاً
        logging.warning("RENDER_EXTERNAL_URL not set yet. Restart later to set webhook.")
        return

    webhook_path = f"/webhook/{WEBHOOK_SECRET}"
    webhook_url = f"{APP_URL}{webhook_path}"
    # سجّل Webhook مع سرّ التوقيع
...
async def telegram_webhook(secret: str, request: Request):
    ...
    return PlainTextResponse("ok")
'''
out = Path("/mnt/data/main_render.py").write_text(code, encoding="utf-8")
print("Wrote:", "/mnt/data/main_render.py")
