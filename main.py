...
async def bind_from_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نحاول نفهم: هل هذه الرسالة "تحويل" (forward) أم لا؟
    is_forward = False
    fchat = None
    fchat_type = None

    # 1) الشكل الكلاسيكي: forward_from_chat
    if getattr(msg, "forward_from_chat", None):
        fchat = msg.forward_from_chat
        fchat_type = fchat.type
        is_forward = True

    # 2) الشكل الجديد: forward_origin.chat
    elif getattr(msg, "forward_origin", None):
        try:
            origin = msg.forward_origin
            fchat = getattr(origin, "chat", None)
            fchat_type = getattr(fchat, "type", None) if fchat else None
            is_forward = True
        except Exception:
            fchat = None

    # 3) كـ fallback: بعض الحالات يكون فيها sender_chat
    if not fchat and getattr(msg, "sender_chat", None) and msg.forward_date:
        # في بعض الأنماط، الرسالة تظهر كمنشور قناة تم نسخه
        fchat = msg.sender_chat
        fchat_type = getattr(fchat, "type", None)
        is_forward = True

    # لو ليست إعادة توجيه إطلاقًا:
    if not is_forward:
        text = (msg.text or msg.caption or "").strip().lower()
        # فقط لو المستخدم يتكلم عن "ربط" نجاوبه، غير كذا نتجاهل
        if any(k in text for k in (" /bind", "/bind", "bind", "ربط")):
            await msg.reply_text(
                "⚠️ هذه ليست *إعادة توجيه* من القناة.\n"
                "لربط القناة بتحويل منشور: افتح القناة → اختر رسالة → **Forward** (بدون Copy) → أرسلها هنا.\n"
                "أو استخدم: `/bind @username`",
                parse_mode=ParseMode.MARKDOWN
            )
        return

    # لو واضح أنها forward لكن ما قدرنا نحدد القناة
    if not fchat:
        await msg.reply_text(
            "⚠️ لم أستطع التعرف على القناة من هذه الرسالة.\n"
            "تأكد أنك استخدمت *Forward* العادي (وليس Copy أو إخفاء المصدر)، "
            "أو استخدم الأمر:\n`/bind @اسم_القناة`",
            parse_mode=ParseMode.MARKDOWN
        )
        return
...
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_done_input":
        ...
        # 👇 نبني الأزرار كاملة من البداية (تفاعل + ملاحظة)
        base_buttons = []
        if use_reactions:
            base_buttons.append([
        ...
        # زر رفع الملاحظة يتطلب chat_id و message_id → نضيفه بعد الإرسال
        keyboard = InlineKeyboardMarkup(base_buttons) if base_buttons else None
        ...
        # ✨ بعد الإرسال فقط نضيف زر الملاحظة (تعديل واحد)
        if sent_message:
            ...
            try:
                ...
                logging.error(f"[PUBLISH] Failed to add note button: {e}")
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"[REACTIONS] user={user_id} chat={chat_id} msg={message_id} data={data}")

    # نقرأ الكيبورد الحالي من الرسالة
    markup = query.message.reply_markup
    if not markup or not markup.inline_keyboard:
        logging.warning("[REACTIONS] لا يوجد inline_keyboard في الرسالة")
    ...
    try:
        ...
        logging.error(f"[REACTIONS] فشل قراءة الأرقام من الأزرار: {e}")
        like_count = 0
        dislike_count = 0
    ...
    if user_id in reacted_set:
        logging.info(f"[REACTIONS] user={user_id} سبق وتفاعل مع هذه الرسالة")
    ...
    try:
        ...
        logging.info(f"[REACTIONS] تم تحديث الأزرار بنجاح: like={like_count}, dislike={dislike_count}")
    except Exception as e:
        logging.error(f"[REACTIONS] Failed to edit message markup: {e}")
        await query.answer("⚠️ لا يمكن تعديل التفاعل في هذا المنشور.", show_alert=True)
        return
